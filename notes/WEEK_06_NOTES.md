...
Production air quality monitoring pipeline that answers two questions automatically
for every hourly PM2.5 reading:
...
Date range                | 2024-01-30 to 2024-04-05
Training rows             | 1,296 (80%)
Test rows                 | 324 (20%)
Maximum PM2.5 spike       | 469 µg/m³ on 2024-02-18 at 04:00
...
-------------------|-------|-------|---------|--------------------|-----------------------
ARIMA (2,0,2) BEST | 9.93  | 8.35  | 100.64% | Forecasting        | Best model
...
Stationarity:
A time series is stationary when mean and variance do not change over time.
...
Feature engineering | Lag features · rolling averages     | Model inputs
...
Logging             | Python logging · rotating handler   | Observability
...
PYTHONPATH=. python -m src.data.preprocessing
PYTHONPATH=. python -m src.features.feature_engineering
PYTHONPATH=. python -m src.models.train
PYTHONPATH=. python -m src.models.evaluate
